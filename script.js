...
const newYear = `Feb 15, ${new Date().getFullYear()} 00:00:00`;
...
function initCountdown() {
  const interval = setInterval(() => {
    ...
    if (remainingTimeInMs < 1000 * 60 * 60 * 24) {
      d.parentNode.style.display = "none";
    }
    if (remainingTimeInMs < 1000 * 11) {
    ...
    if (remainingTimeInMs < 1000) {
      ...
      label.innerHTML = "Happy New Year 2024";
      label.style.fontSize = "100px";
      label.style.animationDuration = "360s";
      let canvas = document.getElementById("canvas");
      canvas.addEventListener("click", showletter);
      audio.play();
    }
  }, 1000);
}
